...
import {
  removeFromHistory,
  clearHistory,
} from "../../utils/redux/slice/historySlice";
import {
  removeFromLikes,
  clearLikes,
} from "../../utils/redux/slice/likesSlice";
import {
  removeFromWatchLater,
  clearWatchLater,
} from "../../utils/redux/slice/watchLaterSlice";
...
const History = ({ tab }) => {
  ...
  const videos =
    tab === "History" ? history : tab === "Like" ? likes : watchLater;
      ...
      <div className={classes.page}>
        <div className={classes["columns-flex"]}>
          ...
          <div className={classes.list}>
            {filterVideos.length > 0 &&
              filterVideos.map((el) => (
                <div className={classes.video}>
                  <Link
                    to={`/videos/${el.video?._id}`}
                    className={classes["video-link"]}
                    state={{ el: el.video }}
                  >
                    <div
                      style={{
                  ...
                  <div className={classes.delete}>
                    <FontAwesomeIcon
                      icon={faTrash}
                      size="xl"
                      onClick={() => handleDelete(el.video._id)}
                    />
                  </div>
                </div>
              ))}
            {filterVideos.length === 0 && (
              <div>
                <p>No videos in record!</p>
                <button className={classes.btn} onClick={handleExplore}>
                  Explore now.
                </button>
              </div>
            )}
